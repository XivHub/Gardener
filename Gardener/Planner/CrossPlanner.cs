...
public static class CrossPlanner
{
    ...
    /// <summary>Same choice as <see cref="Assign"/> — which of the pair is the anchor already on the
    /// ground (or about to be) and which is planted fresh to trigger every cross — but carrying every
    /// bed the anchor already occupies, not just one, since <see cref="PlanFillStep"/> may be topping up
    /// a layout that is already partway planted.</summary>
    ...
    /// left order never decides the outcome and no isolated bed is needed, unlike <see cref="PlanSingleStep"/>'s
    /// single pair. Steps are emitted anchor beds first and cross beds second: an anchor planted while its
    /// ring neighbours are still empty crosses with nothing and stays itself, so only once every anchor is
    /// down does planting a cross bed land it against a same-seed neighbour on both sides. The plan is
    /// re-checked against itself with <see cref="Verify"/> before it is returned, same as
    /// <see cref="PlanSingleStep"/>.
    /// </summary>
    ...
    /// <see cref="PlanSingleStep"/> or <see cref="PlanFillStep"/> builds the plan, and again immediately
    /// before a Plan sweep runs it, since either call can be the one that catches drift since the plan
    /// was drawn on screen.
    /// </summary>
